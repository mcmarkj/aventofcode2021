...
boards = [
    np.array([[int(x) for x in c.split()] for c in b[1:]])
    for b in chunked(lines[1:], 6)
]
...
print(
    sum([n for n in boards[winner_idxs[0]].flatten() if n not in ns[: number_idxs[0]]])
    * ns[number_idxs[0] - 1]
)
print(
    sum(
        [n for n in boards[winner_idxs[-1]].flatten() if n not in ns[: number_idxs[-1]]]
    )
    * ns[number_idxs[-1] - 1]
)
